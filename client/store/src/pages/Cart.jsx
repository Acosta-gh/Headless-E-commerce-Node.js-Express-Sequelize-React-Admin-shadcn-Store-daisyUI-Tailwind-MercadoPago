...
function Cart() {
  const { user } = useAuth();
  const { cart, vaciarCarrito, agregarAlCarrito, quitarDelCarrito, eliminarDelCarrito } = useCart();
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 relative top-20">
            ...
            <li className="py-4 flex justify-between items-center px-4 hover:bg-gray-50 transition-colors duration-150">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => item.cantidad > 1 && quitarDelCarrito(item.id)}
                >
                  -
                </button>
                <button
                  className={`btn btn-xs btn-outline ${item.cantidad >= item.stock ? "opacity-50 cursor-not-allowed" : ""}`}
                  onClick={() => item.cantidad < item.stock && agregarAlCarrito(item, 1)}
                >
                  +
                </button>
                <button
              ...
              <div className="font-bold text-right ml-4">${item.precio * item.cantidad}</div>
            </li>
      ...
      <div className="mt-6 flex gap-4 justify-end">
        <button className="btn btn-error btn-outline hover:btn-error" onClick={vaciarCarrito}>
          Vaciar carrito
        </button>
