...
export function AuthProvider({ children }) {
    ...
    if (LOGOUT_BEFORE_EXPIRATION_MS > 0) {
      ...
      if (warnAt > 0) {
        warningTimerRef.current = setTimeout(() => {
          if (showAlert) {
            showAlert("Tu sesión expira pronto. Guarda tu trabajo o vuelve a iniciar sesión.", "warning");
          }
        }, warnAt);
      }
    }
  ...
  useEffect(() => {
    function handleStorageChange(e) {
      if (e.key === "token") {
        const token = safeReadTokenFromLocalStorage();
        if (!token || getTokenRemainingMs(token) <= 0) {
          doLogout(true);
        } else {
          startTokenWatch(token);
        }
  ...
  const login = async (credentials) => {
    ...
    try {
      ...
      const loggedUser = result.user || (result.token ? buildUserFromToken(result.token) : null);
      setUser(loggedUser);
      ...
      if (loggedUser) {
        try {
          ...
          console.warn("[AuthContext] no se pudo persistir user tras login:", e);
        }
      }
